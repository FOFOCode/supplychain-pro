...
function generarTelemetria(telemetriaAnterior, tempMinPermitida, tempMaxPermitida) {
  ...
  return {
    temperatura: Math.max(tempMinPermitida - 5, Math.min(tempMaxPermitida + 5, temp + randomBetween(-0.5, 0.5))),
...
async function enviarTelemetria(id_envio, posicion, telemetria, timestamp) {
  try {
    ...
    console.log(`📊 Telemetría enviada (${id_envio}): Temp=${telemetria.temperatura.toFixed(2)}°C, Bateria=${telemetria.porcentaje_bateria.toFixed(0)}%`);
 ...
 */
app.post('/api/simulator/incidents/:id_envio/temperatura-alta', async (req, res) => {
  const { id_envio } = req.params;
  const journey = activeJourneys.get(Number(id_envio));
  ...
  try {
    ...
    console.log(`🔴 Incidente de temperatura alta creado para envío ${id_envio}`);
 ...
 * Simula batería baja
 */
app.post('/api/simulator/incidents/:id_envio/bateria-baja', async (req, res) => {
  const { id_envio } = req.params;
  const journey = activeJourneys.get(Number(id_envio));
 ...
 */
app.post('/api/simulator/incidents/:id_envio/geofence-violation', async (req, res) => {
  const { id_envio } = req.params;
  const journey = activeJourneys.get(Number(id_envio));
 ...
 */
app.post('/api/simulator/incidents/:id_envio/volumen-lleno', async (req, res) => {
  const { id_envio } = req.params;
  const journey = activeJourneys.get(Number(id_envio));
  ...
  try {
    ...
    console.log(`🔴 Incidente de volumen lleno creado para envío ${id_envio}`);
    res.json({ success: true, mensaje: 'Incidente creado' });
  } catch (error) {
    console.error('Error creando incidente:', error.message);
    res.status(500).json({ error: error.message });
